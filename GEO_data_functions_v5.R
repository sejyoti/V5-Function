...
geneExpGEOdata = function(GSEID){
  ...
  sampleTitle = readSampleNames(path)
	...
	#geneExpMat = fread("GSE36133_gene-exp.txt")
	# IDConversion = fread(file = "./Other/Entrez_to_gene_symbol.txt", header = T, sep = "\t")
	# geneExpMat = join_all(list(IDConversion, geneExpMat), by = "Gene", match = "all")
	# geneExpMat$Gene = NULL
	# names(geneExpMat)[names(geneExpMat) == 'To'] <- 'Gene'
